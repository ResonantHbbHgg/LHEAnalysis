...
parser.add_argument("--infile", help="File in ROOT format to be read", nargs='?', type=str, default="test2.root")
...
parser.add_argument("--outfile", help="File in ROOT format for output", nargs='?', type=str, default="weighted_2_hbb.root")
...
w_ggHH0_hbb_pt = weightFile.Get("hFrac_ggHH2_8TeV_gr_hbb_p4_pt")
...
hbb_pt = numpy.zeros(1, dtype=float)
hbb_new_pt = numpy.zeros(1, dtype=float)

outtree.Branch('gr_hbb_w_pt', hbb_pt, 'gr_hbb_w_pt/D')
outtree.Branch('gr_hbb_new_w_pt', hbb_new_pt, 'gr_hbb_new_w_pt/D')
...
#    print b2.Px()
    ...
    w_ggHH0_hbb_pt = weightFile.Get("hFrac_ggHH2_8TeV_gr_hbb_p4_pt")
    ibin = w_ggHH0_hbb_pt.FindBin(chain.gr_hbb_p4_pt)
    w = w_ggHH0_hbb_pt.GetBinContent(ibin)
#    w = h1D_pt_data -> GetBinContent(h1D_pt_data -> FindBin(gr_hbb_p4_pt)
#    print 

    spatial_b1 = TVector3(b1.Px()*w, b1.Py()*w, b1.Pz())
#    b1_new = b1.SetVectM(spatial_b1, b1.M())
    b1.SetVectM(spatial_b1, b1.M())
#    print b1.Px()
    spatial_b2 = TVector3(b2.Px()*w, b2.Py()*w, b2.Pz())
    b2.SetVectM(spatial_b2, b2.M())

    spatial_h = TVector3(hbb.Px()*w, hbb.Py()*w, hbb.Pz())
    hbb.SetVectM(spatial_h, hbb.M())
 
    hbb_new = b1 + b2 

    hbb_pt[0] = hbb.Pt()
    hbb_new_pt[0] = hbb_new.Pt()
    if ievt % 1000 == 0:
